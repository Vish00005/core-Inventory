...
export const getDashboardData = async (req, res, next) => {
  try {
    ...
    const allInventory = await Inventory.find().populate('product', 'category reorderLevel');
    ...
    const toBeReceived = pendingReceipts.length > 0 ? pendingReceipts[0].total : 0;
    const toBeDelivered = pendingDeliveries.length > 0 ? pendingDeliveries[0].total : 0;
    ...
    const warehouseInventories = await Inventory.find().populate('warehouse', 'name');
